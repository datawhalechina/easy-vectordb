...
with st.expander("⚙️ 配置参数设置", expanded=True):
    with st.form("config_form"):
        ...
        if submitted:
            ...
            # 发送到后端
            try:
                response = requests.post("http://localhost:8506/update_config", json=config_data)
            ...
            with st.spinner("上传文件中，请稍候..."):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # 发送更新请求
                        update_response = requests.post("http://localhost:8506/update_config", json=config_update)
                    ...
                    if search_response.status_code == 200:
                        ...
                        if col_choice.lower() == "hdbscan" and "clusters" in search_result and search_result["clusters"]:
                            vis_response = requests.post(
                                "http://localhost:8506/visualization",
    ...
    if st.button("🔄 执行切分测试", key="chunking_test_btn"):
        if test_text:
            with st.spinner("正在执行文本切分..."):
                try:
                    response = requests.post(
                        "http://localhost:8506/chunking/process",
                        json={
                            "text": test_text,
...
with st.expander("🔧 系统状态", expanded=False):
    if st.button("📋 获取系统状态", key="system_status_btn"):
        try:
            response = requests.get("http://localhost:8506/system/status")
            if response.status_code == 200:
