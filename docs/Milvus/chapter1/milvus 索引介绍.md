...
当我们提供一个查询向量时，会先计算该向量与所有聚类中心之间的距离，然后选择距离最近的聚类中心，并仅搜索该聚类中心内的向量。通过这种方式，IVF系列索引能够显著减少搜索空间，提高搜索效率。
但这种方法也存在缺点，在进行近似最近邻搜索时，可能会出现一种情况：通过算法找到的“最接近查询嵌入的候选嵌入”实际上并不是真正的最近邻。
...
#### 基于图的索引：HNSW系列
![alt text](../../src/hnsw.png)
HNSW（分层导航小世界图）通过构建多层导航图实现高效向量检索，其核心结构特点为上层稀疏、下层密集，搜索过程从上层起始逐步向下层精确导航，这一设计使其能够实现对数时间复杂度的查询性能，从而具备低延迟优势。在构建阶段，通过参数M（图中节点最大传出连接数量，取值范围[2,2048]）控制图的连接密度，efConstruction（构建时的搜索范围，取值范围[1,int_max]）平衡索引质量与构建时间；搜索阶段则通过参数ef（搜索时的搜索范围，取值范围[top_k,int_max]）调节查询时间与准确度的权衡。
...
综上，用户需根据需求优先级决策：追求高吞吐低延迟（如小topK高召回场景）可选择HNSW；内存资源有限或需处理大topK查询时，IVF_PQ/SQ结合内存映射（mmap）是更优解；磁盘级数据管理则优先考虑DiskANN。
...
| 数据特性 | 数据存储位置 | 内存 / 磁盘          |
| 资源约束 | 内存资源   | 有限 / 充足          |
| 资源约束 | 内存使用优化 | 量化技术 / 磁盘存储      |
| 业务需求 | 召回率要求  | 高召回率 / 可接受召回率下降  |
| 业务需求 | 查询延迟   | 低延迟 / 可接受范围内延迟   |
| 业务需求 | topK大小 | 小批量结果 / 大批量结果    |
| 业务需求 | 过滤率    | 低过滤率 / 高过滤率      |
| 业务需求 | 性能要求   | QPS / 延迟 / 召回率平衡 |



数据特性主要涉及数据的存储位置，例如数据是存储于内存还是磁盘，这直接影响索引的存储设计与访问效率。  

资源约束中，内存资源是核心考量因素，包括内存是否有限或充足，以及是否需要通过量化技术或磁盘存储来优化内存使用 。  

业务需求则涵盖多个关键性能指标，包括召回率要求（如高召回率或可接受一定程度的召回率下降）、查询延迟（如低延迟或可接受范围内的延迟）、topK大小（如小批量或大批量结果返回）、过滤率（如低过滤率或高过滤率需求），以及整体性能要求（如QPS、延迟与召回率的平衡） 。  

在优先级排序上，向量类型通常被视为首要筛选条件，因其决定了索引方法的适用性；在此基础上，内存资源等资源约束构成选择的基础限制，而业务需求中的召回率和查询延迟则作为核心性能目标，进一步细化索引的选型方向。
...
在ANNS向量索引的实际应用中，用户常对索引类型差异、参数配置及量化策略存在疑问，以下结合技术特性进行通俗解释并提供实践建议。  
