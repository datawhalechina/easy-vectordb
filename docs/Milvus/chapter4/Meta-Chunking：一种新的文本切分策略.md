...
1. **模型提供“条件概率预测”的能力**  
   ...
   对于句子 \(x_i\) 中的每个 token \(t_k^i\)，模型需要计算其在“前文所有信息（即 \(t_{<i}\)，也就是 \(x_i\) 之前的所有句子的 token）”和“该句子内部前文 token（\(t_{<k}^i\)）”条件下的出现概率 \(P(t_k^i | t_{<k}^i, t_{<i})\) 。  
...
---
### 熵的近似：香农的 G<sub>K</sub> 公式及其证明 (核心重点)
