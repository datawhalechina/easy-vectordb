## Chapter 4 Milvus实战：基于BM25的混合搜索
...
class MilvusSchemaDesigner:
    ...
    def create_collection(self, vector_dimension: int = 768, drop_existing: bool = False):
        ...
        print(f"\n✓ 集合 '{self.collection_name}' 创建完成！")
        print("\n下一步:")
        print("1. 运行 data_importer.py 导入数据")
        print("2. 运行 hybrid_search.py 进行混合搜索测试")
...
class DataImporter:
    ...
    def load_data_from_json(self, json_path: str) -> List[Dict]:
        ...
        with open(json_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        print(f"✓ 从 {json_path} 加载了 {len(data)} 条数据")
        return data

    def prepare_batch_data(self, raw_data: List[Dict]) -> List[Dict]:
        """
        准备批量插入的数据
        ...
        for idx, item in enumerate(raw_data):
            # 构建数据记录
            record = {
                "id": str(item.get("id", f"doc_{idx}")),
                "embedding": item.get("embedding", []),  # 768维或1024维向量
                "title": item.get("title", "")[:512],     # 限制最大长度
                "content": item.get("content", "")[:65535]  # 限制最大长度
            }

            # 验证数据完整性
            if not record["embedding"] or len(record["embedding"]) == 0:
                print(f"⚠ 跳过第 {idx} 条数据：缺少向量")
                continue

            batch_data.append(record)

        print(f"✓ 准备了 {len(batch_data)} 条有效数据")
        return batch_data

    def insert_data(self, batch_data: List[Dict], batch_size: int = 100):
        """
        批量插入数据到Milvus
        ...
        for i in range(0, total, batch_size):
            ...
            try:
                ...
                print(f"✗ 插入失败 (batch {i}-{i+len(batch)}): {e}")
        ...
        print(f"  平均速度: {total/elapsed_time:.2f} 条/秒")
    ...
    def get_collection_stats(self):
        ...
        try:
            ...
            print(f"\n集合统计信息:")
            print(f"  总文档数: {stats.get('row_count', 0)}")
        except Exception as e:
            print(f"获取统计信息失败: {e}")
...
def main():
    ...
    try:
        # 1. 加载数据
        json_path = "data/processed_gov_qa.json"  # 修改为实际路径
        raw_data = importer.load_data_from_json(json_path)

        # 2. 准备数据
        batch_data = importer.prepare_batch_data(raw_data)

        # 3. 插入数据
        if batch_data:
            importer.insert_data(batch_data, batch_size=100)
