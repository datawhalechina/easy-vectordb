...
def run_experiment_base(config_params: Dict[str, Any]) -> Dict[str, Any]:
  ...
  opt_start = time.time()
    # 只传递 optimize_embeddings 明确需要的参数
  opt_results = optimize_embeddings(
    ...
    d=config_params["d"],
    k=config_params["k"],
    learning_rate=config_params.get("learning_rate", 0.01),
    num_iterations=config_params.get("num_iterations", 100000),
    temperature=config_params.get("temperature", 0.1),
    seed=config_params.get("seed", 42),
    show_progress=config_params.get("show_progress", True),
    log_interval=config_params.get("log_interval", 50),
    early_stopping_patience=config_params.get("early_stopping_patience", 1000),
    early_stopping_min_delta=config_params.get("early_stopping_min_delta", 0.00001),
    early_stopping_monitor_metric=config_params.get("early_stopping_monitor_metric", "loss"),
    early_stopping_restore_best_weights=config_params.get("early_stopping_restore_best_weights", False)
  )
