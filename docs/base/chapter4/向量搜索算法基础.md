...
那么问题来了，怎么定义这个向量相似性呢？我们抽象成一个数学问题，现在给定一个查询向量 q（代表"需要查询的商品"），在一个庞大的向量集合 {v_1, v_2, ..., v_n}（代表"商品库中待比较的商品向量"，通常 n 在百万到十亿甚至万亿级）中，快速找到与 q 最相似的前 K 个向量（代表"推荐给用户的K个最相似商品"，K-nearest neighbors, KNN）。
...
3. 在计算复杂度上，L2和IP的计算复杂度都是O(d)，其中d是向量的维度。因此，在维度较高的情况下，两者的计算复杂度是相同的。
    ...
    **但是！我们来结合公式分析一下两者的计算步骤：**
  
    给定两个 d 维向量 A = `[a₁, a₂, ..., a_d]` 和 B = `[b₁, b₂, ..., b_d]`。
    ...
    3.1.  L2距离的公式为：
      ...
      其计算步骤分解为：
      *   **计算差值向量** C = A - B -> 对应公式中的 `(a_i - b_i)`，需要 **d 次减法**。
      *   **计算平方** -> 对应公式中的 `(...)^2`，需要 **d 次乘法**。
      *   **求和** -> 对应公式中的 `∑`，需要 **(d-1) 次加法**。
      *   **开平方根** -> 对应公式中的 `√`，需要 **1 次**相对昂贵的运算。

    3.2.  内积(IP)的公式为：
      ```math
      \text{IP}(A, B) = \sum_{i=1}^{d} a_i \times b_i
      ```
      其计算步骤分解为：
      *   **计算对应项乘积** -> 对应公式中的 `a_i × b_i`，需要 **d 次乘法**。
      *   **求和** -> 对应公式中的 `∑`，需要 **(d-1) 次加法**。

    **复杂度计算总结：**
    *   **IP** 的操作数为：**d次乘法 + (d-1)次加法**，时间复杂度为 **O(d)**。
    *   **L2** 的操作数为：**d次减法 + d次乘法 + (d-1)次加法 + 1次开方**，时间复杂度也为 **O(d)**。

    你可以看到，虽然两者的渐进复杂度相同，但在具体操作上 **L2 比 IP 多做了 d 次减法和 1 次开平方根运算**。对于单次计算，L2的开销要比IP要高，但这种差异在绝大多数场景下均是可忽略的。主要的时间消耗在于 **O(d)** 量级的乘法和加法。
